        ...
        <table class="table table table-bordered table-striped table-hover table-condensed tfix mb-1" style="font-family: Arial; font-size: 13px">
          ...
          <tstatus>
		   @foreach($group_roles as $i=>$grouprole)
		         ...
		         <td>
					<a class="btn btn-info btn-sm" href='{{URL::action("admin\GroupRoleController@edit",array($grouprole->id))}}'><i class="os-icon os-icon-ui-49" style="color: white"></i></a>
					<a class="btn btn-primary btn-sm" href='{{URL::action("admin\GroupRoleController@show",array($grouprole->id))}}'><i class="os-icon os-icon-grid-10"></i></a>
					<form class="btn btn-danger btn-sm" id="delete_grouprole{{$grouprole->id}}" action='{{URL::action("admin\GroupRoleController@destroy",array($grouprole->id))}}' method="POST">
						...
						<a href="#" onclick="document.getElementById('delete_grouprole{{$grouprole->id}}').submit();"><i class="os-icon os-icon-ui-15" style="color: white"></i></a>
					</form>
				</td>	         
		     	</tr>
			   @endforeach
